...
assert.equal((html.match(/src="\/sapuri-brand-logo\.webp"/g) || []).length, 3, 'หน้าบัญชีและเลือกคลังต้องใช้โลโก้ SAPURI ใหม่');
assert.doesNotMatch(html, /class="rx">Rx<\/span>/, 'ต้องไม่แสดงตรา Rx เดิมในหน้าเข้าสู่ระบบ');
assert.match(html, /<title>SAPURI<\/title>/);
assert.match(html, /<link rel="icon" href="\/sapuri-app-icon-192\.png" type="image\/png">/);
