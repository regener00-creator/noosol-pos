...
assert.match(html, /const MOBILE_SCAN_ERROR_SOUND_URL='data:audio\/mpeg;base64,/);
assert.doesNotMatch(html, /\/mobile-scan-success\.mp3/);
assert.doesNotMatch(html, /\/mobile-scan-error\.mp3/);
const embeddedScanSound=html.match(/const MOBILE_SCAN_SOUND_URL='data:audio\/mpeg;base64,([^']+)'/);
const embeddedScanErrorSound=html.match(/const MOBILE_SCAN_ERROR_SOUND_URL='data:audio\/mpeg;base64,([^']+)'/);
assert.ok(embeddedScanSound&&Buffer.from(embeddedScanSound[1],'base64').length>1000);
assert.ok(embeddedScanErrorSound&&Buffer.from(embeddedScanErrorSound[1],'base64').length>1000);
...
assert.match(html, /class="mobile-camera-head"><button[^>]+class="mobile-camera-close"[^>]*>×<\/button><span>P R A N C - H I B E S<\/span>/);
...
assert.doesNotMatch(mobileStockPanel, /id="mobileStockInput"/);
...
assert.match(html, /auth:\{persistSession:true,autoRefreshToken:true\}/);
assert.match(html, /function prepareMobileLandingPage\(\)\{[\s\S]{0,120}mobileToolMode='price'/);
assert.match(html, /if\(currentProfile&&isMobileDeviceMode\(\)\) prepareMobileLandingPage\(\)/);
...
const mobileLogoIndex = mobileFooter.indexOf('mobile-tools-logo"><img src="/sapuri-brand-logo.webp"');
...
assert.match(html, /<link rel="manifest" href="\/manifest\.webmanifest\?v=__PEPOS_ASSET_VERSION__">/);
...
const mobilePriceEditableStart = mobilePriceResult.indexOf('mobile-price-edit-grid');
const mobilePriceEditableEnd = mobilePriceResult.indexOf('mobile-price-edit-save', mobilePriceEditableStart);
const mobilePriceEditable = mobilePriceResult.slice(mobilePriceEditableStart, mobilePriceEditableEnd);
const mobilePriceEditableSaleIndex = mobilePriceEditable.indexOf('mobilePriceEditSale');
const mobilePriceEditableCostIndex = mobilePriceEditable.indexOf('mobilePriceEditCost');
const mobilePriceEditableUnitIndex = mobilePriceEditable.indexOf('mobile-unit-select');
const mobilePriceEditableLotIndex = mobilePriceEditable.indexOf('mobile-price-lot-field');
assert.ok(mobilePriceNameIndex >= 0 && mobilePriceStockIndex > mobilePriceNameIndex);
assert.ok(mobilePriceEditableSaleIndex >= 0 && mobilePriceEditableCostIndex > mobilePriceEditableSaleIndex && mobilePriceEditableUnitIndex > mobilePriceEditableCostIndex && mobilePriceEditableLotIndex > mobilePriceEditableUnitIndex);
const mobilePriceReadonlyStart = mobilePriceResult.indexOf('<div class="mobile-metrics">');
const mobilePriceReadonly = mobilePriceResult.slice(mobilePriceReadonlyStart);
const mobilePriceReadonlyStockIndex = mobilePriceReadonly.indexOf('<span>คงเหลือ</span>');
const mobilePriceReadonlySaleIndex = mobilePriceReadonly.indexOf('<span>ราคาขาย</span>');
const mobilePriceReadonlyCostIndex = mobilePriceReadonly.indexOf('<span>ทุน</span>');
const mobilePriceReadonlyUnitIndex = mobilePriceReadonly.indexOf('mobile-unit-select');
const mobilePriceReadonlyExpiryIndex = mobilePriceReadonly.indexOf('<span>วันหมดอายุ</span>');
assert.ok(mobilePriceReadonlyStockIndex >= 0 && mobilePriceReadonlySaleIndex > mobilePriceReadonlyStockIndex && mobilePriceReadonlyCostIndex > mobilePriceReadonlySaleIndex && mobilePriceReadonlyUnitIndex > mobilePriceReadonlyCostIndex && mobilePriceReadonlyExpiryIndex > mobilePriceReadonlyUnitIndex);
...
assert.match(html, /\.mobile-price-edit-grid>\.mobile-unit-select,\.mobile-metrics>\.mobile-unit-select\{grid-column:1\/-1;/);
assert.match(html, /data-mobile-tool="inventory">ตรวจและแก้ไขสต๊อก<\/button>/);
assert.match(html, /<\/nav>\s*<div class="mobile-tools-tab-brand">P R A N C - H I B E S<\/div>/);
assert.match(html, /\.mobile-tools-tab-brand\{[^}]*text-align:center;[^}]*font:600 9px\/1\.2/);
...
assert.ok(manifest.icons.some(icon => icon.src === '/sapuri-app-icon-192.png' && icon.sizes === '192x192'));
assert.ok(manifest.icons.some(icon => icon.src === '/sapuri-app-icon-512.png' && icon.sizes === '512x512'));
assert.match(serviceWorker, /request\.mode==='navigate'/);
assert.match(serviceWorker, /fetch\(request\)/);
assert.match(serviceWorker, /caches\.match\('\/index\.html'\)/);
assert.match(serviceWorker, /CACHE_NAME=`pepos-mobile-\$\{ASSET_VERSION\}`/);
...
assert.ok(fs.statSync(path.join(root, 'sapuri-app-icon-192.png')).size > 3000);
assert.ok(fs.statSync(path.join(root, 'sapuri-app-icon-512.png')).size > 3000);
...
assert.equal(soundContext.playMobileScanErrorSound(), true);
assert.equal(soundContext.playMobileScanErrorSound(), true);
assert.equal(sounds.length, 2);
assert.equal(sounds[1].src, 'data:audio/mpeg;base64,RVJST1I=');
assert.equal(sounds[1].preload, 'auto');
assert.equal(sounds[1].loadCount, 1);
