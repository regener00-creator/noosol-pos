...
assert.match(html, /if\(!continuous\)\{ controls\?\.stop\?\.\(\); closeMobileCameraScanner\(\); onCode\(value\); return true; \}/);
...
assert.match(html, /บน iPhone\/iPad ให้เปิดด้วย Safari แล้วเพิ่มไปยังหน้าจอโฮม/);
assert.match(html, /function ensureMobileZxingLoaded\(\)/);
assert.match(html, /ZXingBrowser\?\.BrowserMultiFormatReader/);
assert.match(html, /new zxing\.BrowserMultiFormatReader/);
assert.doesNotMatch(html, /เบราว์เซอร์นี้ยังสแกนด้วยกล้องไม่ได้ กรุณาเปิดใน Chrome/);
