...
(async()=>{
  ...
  await page.evaluate(async()=>{
    ...
    window.loyaltyRpc=async(name,p)=>{
      ...
      if(name==='complete_sale'){
        ...
        return {data:{sale:{...p.p_sale,id:'TEST-LOYALTY',loyalty:{earned:Math.floor(p.p_sale.total/100),redeemed:p.p_sale.loyaltyRedeemed,periodStart:'2026-09-03',expiresOn:'2027-09-03',balanceAfter:109}}}};
      }
  ...
  assert.equal(await page.locator('.loyalty-account-summary').innerText(),'200 แต้ม\n/ หมดอายุ 03-12-2026');
  const balanceBox=await page.locator('.loyalty-account-summary .loyalty-balance').boundingBox();
  const expiryBox=await page.locator('.loyalty-account-summary .loyalty-expiry-date').boundingBox();
  assert.ok(Math.abs((balanceBox.y+balanceBox.height)-(expiryBox.y+expiryBox.height))<3,'balance and expiry must stay on the same line');
  assert.equal(await page.locator('.loyalty-points-rule').innerText(),'• 100 บาท = 1 แต้ม / ใช้แต้มได้เมื่อยอดถึง 1,000 บาท');
  ...
  await page.locator('.pos-customer-history-modal .customer-purchases-page').waitFor();
  await page.getByText('RE-1',{exact:true}).waitFor();
  assert.equal(await page.locator('.pos-customer-history-modal').getAttribute('role'),'dialog');
  assert.equal(await page.locator('#customerLoyaltyPanel').count(),1);
  assert.equal(await page.locator('#posCustomerHistoryLoyaltyPanel').count(),1);
  assert.deepEqual(await page.evaluate(()=>({currentTab,modalOpen:posCustomerHistoryModalOpen,originTab:customerHistoryView?.originTab,cartCount:cart.length,customerId:saleMember?.id})),{currentTab:'checkout',modalOpen:true,originTab:'checkout',cartCount:1,customerId:1});
  await page.locator('#closeCustomerHistory').click();
  await page.locator('.pos-customer-history-overlay').waitFor({state:'detached'});
  assert.deepEqual(await page.evaluate(()=>({currentTab,modalOpen:posCustomerHistoryModalOpen,cartCount:cart.length,customerId:saleMember?.id})),{currentTab:'checkout',modalOpen:false,cartCount:1,customerId:1});
  ...
  assert.match(await page.locator('.customer-tier-progress-pos').innerText(),/ลูกค้าทั่วไป[\s\S]*ซื้ออีก 10,000.00 บาท ถึงลูกค้าประจำ/);
  assert.equal(await page.locator('.customer-tier-progress-pos [role="progressbar"]').getAttribute('aria-valuenow'),'90');
  ...
  assert.equal(result.sale.total,900);assert.equal(result.sale.loyalty.earned,9);assert.equal(result.cart,0);assert.equal(result.pending,null);
