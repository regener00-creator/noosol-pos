...
(async()=>{
  ...
  await page.evaluate(()=>{
    ...
    fetchAllRows=async()=>({data:window.remoteRows.filter(r=>r.data.dataReviewStatus).map(r=>({id:r.id,...r.data})),error:null});
  ...
  assert.deepEqual(await page.evaluate(()=>window.fullRowsRequested),[[9101]],'fetch full details only for changed colors');
  await page.evaluate(()=>{productReviewRefreshAt=0;window.remoteRows[0].data={};window.remoteRows[0].revision=10;});
  await page.evaluate(()=>refreshProductReviewColors());
  assert.equal(await page.locator('.product-review-pending-row').count(),0,'receive cleared color');
  await page.evaluate(()=>{productReviewRefreshAt=0;window.remoteRows[0].data={dataReviewStatus:'complete'};window.remoteRows[0].revision=11;productDirtyOperations.set('9101','update');});
  await page.evaluate(()=>refreshProductReviewColors());
  ...
  await page.evaluate(()=>{window.remoteRows[0].data={dataReviewStatus:'pending'};window.remoteRows[0].revision=12;});
  await page.waitForFunction(()=>document.querySelector('.product-review-pending-row'),{},{timeout:22000});
  assert.equal(await page.evaluate(()=>products[0]._revision),12,'scheduled receive updates a desktop list left open');
