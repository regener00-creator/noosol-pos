...
const withFood=context.normalizeDispensingLabel({...structuredInput,mealTiming:'with_food'});
assert.equal(withFood.directions, 'รับประทานครั้งละ 1 เม็ด พร้อมอาหาร เช้า กลางวัน เย็น เป็นเวลา 7 วัน');
assert.match(context.medicineLabelScheduleHtml(withFood), /<i class="is-checked"><\/i>พร้อมอาหาร/, 'ตัวเลือกพร้อมอาหารต้องแสดงบนฉลากพิมพ์');
const emptyStomach=context.normalizeDispensingLabel({...structuredInput,mealTiming:'empty_stomach'});
assert.equal(emptyStomach.directions, 'รับประทานครั้งละ 1 เม็ด ขณะท้องว่าง เช้า กลางวัน เย็น เป็นเวลา 7 วัน');
assert.match(context.medicineLabelScheduleHtml(emptyStomach), /<i class="is-checked"><\/i>ขณะท้องว่าง/, 'ตัวเลือกขณะท้องว่างต้องแสดงบนฉลากพิมพ์');
...
assert.match(printHtml, /\.medicine-label-schedule \.medicine-label-cell:first-child\{justify-content:flex-start\}/, 'ตัวเลือกมื้ออาหารต้องเริ่มต่อจากไอคอน');
assert.match(printHtml, /\.medicine-label-meal-choice-grid\{[^}]*grid-template-columns:repeat\(3,minmax\(0,1fr\)\)/, 'ตัวเลือกมื้ออาหารบนฉลากต้องเรียงเป็นตาราง');
...
assert.match(html, /value:'before',label:'ก่อนอาหาร'/, 'ต้องมีตัวเลือกก่อนอาหาร');
assert.match(html, /value:'after',label:'หลังอาหาร'/, 'ต้องมีตัวเลือกหลังอาหาร');
assert.match(html, /type="checkbox" name="medicineLabelMealTiming" value="\$\{option\.value\}"/, 'ตัวเลือกมื้ออาหารต้องกดซ้ำเพื่อเอาติ๊กออกได้');
assert.match(html, /value:'with_food',label:'พร้อมอาหาร'/, 'ต้องมีตัวเลือกพร้อมอาหาร');
...
assert.match(html, /id="medicineLabelWarningPresetPopupTitle">ข้อความ Quick Use<\/h3>/, 'ต้องจัดการ Quick Use ใน Popup');
assert.doesNotMatch(html, /id="medicineLabelWarningPresetManage"/, 'ต้องนำปุ่มจัดการเดิมออกจากหน้าหลัก');
assert.match(html, /data-medicine-warning-preset-action="up"/, 'ต้องเลื่อนลำดับคำเตือน Quick Use ขึ้นได้');
assert.match(html, /data-medicine-warning-preset-action="down"/, 'ต้องเลื่อนลำดับคำเตือน Quick Use ลงได้');
assert.match(html, /data-medicine-warning-preset-action="delete"/, 'ต้องลบคำเตือน Quick Use ได้');
assert.match(html, /function normalizeMedicineLabelWarningPresets\(/, 'ต้องตรวจและตัดคำเตือน Quick Use ที่ซ้ำกัน');
assert.match(html, /function saveMedicineLabelWarningPresets\(/, 'ต้องบันทึกคำเตือน Quick Use ที่จัดเอง');
assert.match(html, /medicineLabelWarningPresets:\[\.\.\.MEDICINE_LABEL_WARNING_PRESETS\]/, 'ข้อมูลธุรกิจเริ่มต้นต้องมีคำเตือน Quick Use มาตรฐาน');
