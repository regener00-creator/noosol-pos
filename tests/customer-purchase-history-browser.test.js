...
(async()=>{
  ...
  assert.deepEqual(firstCustomerCells.slice(0,7),['ลูกค้าชื่อเหมือนกัน','0812345678','line-1','25 แต้ม','ลูกค้าประจำ','10,000.00 บาท','11-09-2027']);
  ...
  assert.equal(await page.locator('.customer-tier-choice.is-current').textContent(),'ลูกค้าประจำ');
  assert.equal(await page.locator('.customer-tier-current-average').count(),0);
  assert.match(await page.locator('.customer-tier-progress-history').innerText(),/ความคืบหน้าสู่ระดับถัดไป[\s\S]*ซื้ออีก 360,000.00 บาท ถึงลูกค้าพิเศษ/);
  assert.equal(await page.locator('.customer-tier-progress-history [role="progressbar"]').getAttribute('aria-valuenow'),'20');
  ...
  await page.locator('#refreshCustomerPurchases').click();
  await page.locator('.customer-tier-choice-regular.is-current').waitFor();
