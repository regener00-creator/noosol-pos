...
(async()=>{
  ...
  assert.deepEqual(await page.locator('.customer-tier-choice').allTextContents(),['ลูกค้าทั่วไป','ลูกค้าประจำยอดซื้อเฉลี่ย 10,000.00 บาท✓ ระดับนี้','ลูกค้าพิเศษ']);
  assert.equal(await page.locator('.customer-tier-choice.is-current').getAttribute('aria-current'),'true');
  assert.equal(await page.locator('.customer-tier-choice.is-current').textContent(),'ลูกค้าประจำยอดซื้อเฉลี่ย 10,000.00 บาท✓ ระดับนี้');
  assert.equal(await page.locator('.customer-tier-current-average').innerText(),'ยอดซื้อเฉลี่ย\n10,000.00 บาท');
