...
const recoveryMigrationName = fs.readdirSync(path.join(root, 'supabase', 'migrations')).find((name) => name.endsWith('_password_recovery_questions.sql'));
const recoveryMigration = fs.readFileSync(path.join(root, 'supabase', 'migrations', recoveryMigrationName), 'utf8');
...
assert.match(recoveryMigration, /create table if not exists public\.password_recovery_challenges/i);
assert.match(recoveryMigration, /record_password_recovery_failure/i);
assert.match(recoveryMigration, /enable row level security/i);
assert.match(recovery, /password_recovery_challenges/);
assert.match(recovery, /PBKDF2/);
assert.match(recovery, /timingSafeEqualHex/);
assert.match(recovery, /record_password_recovery_failure/);
assert.doesNotMatch(recovery, /owner_recovery_codes/);
