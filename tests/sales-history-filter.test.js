...
assert.match(html, /function renderPOSSalesHistoryModal\(\)/);
assert.match(html, /class="modal pos-sales-history-modal"/);
assert.match(html, /id="closePOSSalesHistoryBottomBtn"/);
assert.match(html, /histBtn\.addEventListener\('click', \(\)=>\{ posSalesHistoryModalOpen=true; render\(\); \}\)/);
assert.doesNotMatch(html, /<h1>ประวัติการขาย<\/h1>/);
