...
const cashBillNavIndex = navSource.indexOf("['cashbill','บิลเงินสด'");
const productsNavIndex = navSource.indexOf("['products','PRODUCT'");
const stockControlNavIndex = navSource.indexOf("['stockcontrol','ตรวจนับ / ปรับสต๊อก'");
const barcodePrintNavIndex = navSource.indexOf("['barcodeprint','พิมพ์ป้ายราคา'");
const warehouseNavIndex = navSource.indexOf("['warehouse','ตั้งค่าคลังสินค้า'");
...
const settingsUsersNavIndex = navSource.indexOf("['settingsusers','ตั้งค่าผู้ใช้งาน'");
...
assert.ok(auditLogNavIndex > systemSettingsNavIndex, 'AUDIT LOG ต้องอยู่ล่างสุดของกลุ่มตั้งค่า');
...
Object.assign(context, {
  ...
  AUTOMATIC_STOCK_ADJUSTMENT_REASON: 'ตรวจนับและปรับสต๊อกจากยอดตรวจนับ',
...
assert.match(rendered, /<th>บาร์โค้ด<\/th><th>สินค้า<\/th><th>หน่วย<\/th><th>ในระบบ<\/th><th>นับได้<\/th><th>ต่าง<\/th><th>LOT ที่ปรับ<\/th><th aria-label="จัดการ"><\/th>/);
assert.doesNotMatch(rendered, /<th>รหัสสินค้า<\/th>/);
assert.match(context.stockEditRowsHtml([]), /colspan="8"/);
assert.doesNotMatch(rendered, /id="stockEditReason"/);
assert.doesNotMatch(rendered, /id="stockEditNote"/);
assert.doesNotMatch(rendered, /stock-control-meta/);
...
assert.doesNotMatch(rendered, /P-001/);
...
let lastAdjustmentPayload = null;
Object.assign(context, {
