...
(async () => {
  ...
  assert.equal(await page.locator('[data-sales-representative-delete]').count(),5,'every representative card must have a delete-selection checkbox');
  assert.equal(await page.locator('#deleteSelectedSalesRepresentativesBtn').isDisabled(),true);
  await page.locator('[data-sales-representative-delete="10"]').check();
  assert.equal(await page.locator('.representative-profile-panel').count(),0,'checking a representative must not open its detail page');
  assert.equal((await page.locator('#deleteSelectedSalesRepresentativesBtn').textContent()).trim(),'ลบที่เลือก (1)');
  page.once('dialog',dialog=>dialog.dismiss());
  await page.locator('#deleteSelectedSalesRepresentativesBtn').click();
  assert.equal(await page.locator('.representative-group-card').count(),5,'canceling bulk delete must keep every representative');
  await page.locator('[data-sales-representative-delete="10"]').uncheck();
