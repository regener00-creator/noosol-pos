...
assert.match(form, /isNewCustomer[\s\S]*readonly placeholder="ระบบจะสร้างให้อัตโนมัติ"[\s\S]*isNewCustomer\?'':`<div class="contact-editor-field"><label>เครดิต<\/label>/);
...
assert.match(save, /recordId=contactId==='new'\?generateClientRecordId\(contacts\)[\s\S]*contactId==='new'&&fixedType==='customer'\?`C-\$\{Number\(recordId\)\.toString\(36\)\.toUpperCase\(\)\}`/);
