...
assert.match(noteMetadataMigration,/create or replace function public\.get_representative_note_metadata/);
assert.match(noteMetadataMigration,/row_number\(\) over[\s\S]*order by note\.created_at asc nulls first,note\.id asc/);
...
assert.match(html, /\.representative-note-body\{display:-webkit-box;[^}]*overflow:hidden;[^}]*-webkit-line-clamp:5;/);
assert.match(historyGroupRender, /group\.notes\.slice\(0,3\)/);
assert.match(historyGroupRender, /representative-note-overflow/);
assert.match(historyGroupRender, /representativeNoteNumber\(note,fallbackNumber\)/);
...
assert.match(html, /\.representative-profile-products-trigger\{[^}]*background:transparent[^}]*color:var\(--primary\)/, 'กล่องสินค้าที่ดูแลต้องใช้รูปแบบพื้นโปร่งเดิม');
...
assert.match(html, /\.representative-note-search\{grid-template-columns:minmax\(0,1fr\) auto auto auto;\}/);
...
assert.match(noteEditor, /<label for="repActivityTitle">NOTE \$\{noteNumber\}<\/label>/);
assert.doesNotMatch(noteEditor, /representative-note-inline-caption|<label for="repActivityTitle">ชื่อโน้ต<\/label>|representativeNoteCaptionTitle|representativeNoteCaptionDate/);
...
assert.match(html, /function representativeNoteDateLabel\(value\)/);
assert.match(html, /NOTE \$\{representativeNoteNumber\(note,fallbackNumber\)\} : \$\{escapeHtml\(note\.title\|\|'-'\)\}[\s\S]*representativeNoteDateLabel\(note\.eventDate\)/);
