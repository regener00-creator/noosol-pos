...
(async () => {
  ...
  assert.equal(await page.locator('#medicineLabelDoseUnitPopupTitle').textContent(), 'จัดการหน่วยรับประทาน');
  assert.equal(await page.locator('#medicineLabelDoseUnit').inputValue(), selectedDoseUnitBeforeManage, 'เปิด Popup แล้ว Dropdown ต้องกลับไปที่หน่วยเดิม');
  ...
  await page.locator('#medicineLabelDoseUnitAdd').click();
  await page.locator('#medicineLabelDoseUnit').selectOption('หลอด');
  page.once('dialog', dialog=>dialog.accept());
  await page.locator('.medicine-label-unit-row').filter({hasText:'หน่วยชั่วคราว'}).locator('[data-dose-unit-action="delete"]').click();
  assert.equal(await page.locator('#medicineLabelDoseUnit option', {hasText:'หน่วยชั่วคราว'}).count(), 0, 'ต้องลบหน่วยจาก Popup ได้');
  await page.locator('.medicine-label-dose-unit-modal').screenshot({path:path.join(os.tmpdir(),'pepos-medicine-label-dose-unit-popup.png')});
  await page.locator('#medicineLabelDoseUnitPopupDone').click();
  assert.equal(await page.locator('.medicine-label-dose-unit-modal').count(), 0, 'ปุ่มปิดต้องปิด Popup จัดการหน่วย');
  ...
  const warningToolsLayout=await page.locator('.medicine-label-warning-group').evaluate(group=>{
    const list=group.querySelector('#medicineLabelWarningQuickList').getBoundingClientRect();
    const button=group.querySelector('#medicineLabelWarningPresetOpen').getBoundingClientRect();
    return {listBottom:list.bottom,buttonTop:button.top};
  });
  assert.ok(warningToolsLayout.buttonTop>=warningToolsLayout.listBottom, 'ปุ่มเพิ่มข้อความต้องอยู่ท้ายรายการ Quick Use');
